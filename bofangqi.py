...
MingLing_Biao = {
    ...
    "bf_wusun":"yinyue_wav",
    "bf_shici":"shici",
    "bf_xiangsheng":"xiangsheng",
    "bf_pingshu":"pingshu",
    "bf_jiangtan":"jiangtan",
    "bf_langdu":"langdu",
...
def ZhiXing(MoShi, MingLing):
    ZhuanJi = ""
    ZhuanJiQuMu = []
    # check WenJian, whether MingLing same as pre, decide Biao&XuHao(XuHao has to be format of tuple)
    ...
    if MingLing == "bf_suiji":
        SuiJi_Biao = str(list(range(1,int(Max_Id)+1))).strip('[]').split(', ')
    ...
    while True:
        ...
        #check if completed or stop at a certain time
        while True:
        ...
        # check if MoShi has dynamically changed
        get_moshi = MoShi()
        if get_moshi[1]:
